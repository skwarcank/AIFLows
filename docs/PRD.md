...
Build AIFlows as a SaaS product using Next.js and Supabase. The historical local prototype has been removed from active source; git history is the archive.
...
Use the Next.js SaaS app in this repo as the active application. The old Express/Vite local prototype has been removed from active source so agents and developers have one current product shell to reason about.
...
- Supabase migrations;
- `.env.example`;
- one setup guide at `docs/setup.md`;
...
- restoring or improving the historical local Express/Vite prototype unless explicitly requested.
